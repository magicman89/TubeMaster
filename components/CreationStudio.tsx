...
const WaveformVisualizer: React.FC<{
    ...
    zoom?: number
}> = ({ waveform, peaks, subtlePeaks = [], segments, scenes, currentTime = 0, duration = 1, height = 64, interactive = false, onSeek, onSceneResize, zoom = 1 }) => {
...
const SequentialPlayer: React.FC<{
    scenes: Scene[],
    projectTitle: string,
...
}> = ({ scenes, projectTitle, aspectRatio, waveform, audioPeaks, audioSubtlePeaks, audioSegments, audioDuration, audioUrl, onSceneUpdate, onSceneSplit, onSceneReorder, onSceneDelete }) => {
    ...
    const handleSeek = (time: number) => {
        setCurrentTime(time);
        // Sync audio if connected (mocked here)
    };
    ...
    return (
        <div className="flex flex-col h-full bg-black/20 rounded-2xl overflow-hidden border border-white/5">
            ...
            <div className="flex-1 relative bg-black flex items-center justify-center overflow-hidden">
                <div
                    className={`relative transition-all duration-300 shadow-2xl ${aspectRatio === '16:9' ? 'w-full max-w-4xl aspect-video' : 'h-full max-h-[600px] aspect-[9/16]'}`}
                >
                    {activeScene?.videoUrl ? (
                        <video
                            ...
                            loop
                            muted // Muted for preview as audio is separate
                            autoPlay={isPlaying}
                        />
                    ) : (
                            ...
                            <div className="flex flex-col gap-2">
                                {['trending', 'news', 'evergreen'].map(s => (
                                    <label key={s} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all ${source === s ? 'bg-blue-600/20 border-blue-500 text-white' : 'bg-white/5 border-white/5 text-slate-400'}`}>
                                        <input type="radio" name="source" checked={source === s} onChange={() => setSource(s as any)} className="hidden" />
...
const CreationStudio: React.FC<CreationStudioProps> = ({ activeChannel, initialPrompt }) => {
    ...
    const addSystemLog = (message: string) => {
        setSystemLogs(prev => {
            const newLog = `[${new Date().toLocaleTimeString([], { hour12: false, fractionalSecondDigits: 1 } as any)}] ${message}`;
    ...
    const startAutopilot = async (config?: AutopilotConfig) => {
        ...
        try {
            ...
            const limitedScenes = (plan.scenes || []).slice(0, 10).map((s: any) => ({ ...s, status: 'pending' }));
